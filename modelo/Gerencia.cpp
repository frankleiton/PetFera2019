...
bool Gerencia::CadastrarAnimal(Animal *animal)
{
    ...
    repo->insert_table_animal(animal->getClasse(),animal->getNome(),animal->getNome_cientifico(),animal->getSexo(),to_string(animal->gettamanho()),animal->getDieta(),to_string(animal->getVeterinario().getID()),to_string(animal->getTratador().getID()),animal->getNome());
}
